...
def main():
    ...
    #activities = ["退職", "月給（ドル）"]
    #choice = st.sidebar.selectbox("グラフのｘ軸", activities)
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            st.caption('')
            st.caption('決定木の可視化')
            st_display_dtree(train_X, train_Y)
    ...
    if choice == 'About':       #About
        ...
        image = Image.open('image.jpg')     #画像読み込み
        st.image(image)

        st.markdown("Dr. Martens")      #見出し設定
        st.text("Version 0.1")

        st.markdown("For More Information check out   (https://jp.drmartens.com/)")     #Dr. Martens URL
